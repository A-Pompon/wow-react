...
const UserClassementDetails = () => {
  ...
  useEffect(() => {
    usersService
      .getMyFriends()
      .then((res) => {
        console.log("friendListUser", res.data);
              ...
              <div className="detail-container">
                ...
                <div className="detail-info">
                  <p className="detail-name">
                    Nom : {user.user_id === null ? "BUGGY" : user.name}
                  </p>
                  <p className="detail-role">
                    Race :{" "}
                    {user.user_id === null ? "BUGGY" : user.race.toUpperCase()}
                  </p>
                </div>
                <div className="detail-score">
                  <p className="score">Score : {user.points}</p>
                  <p className="ratio">
                    Ratio : {(user.victories / user.defeats || 0).toFixed(2)}
                  </p>
                </div>
                <div className="detail-action">
                  ...
                  {idConnectedUser.userId != userIdParams && user && (
                    <div className="button-user">
                      {friendsListUserCo.some(
                        (friend) => friend.Followed.followed_id == userIdParams
                      ) ? (
                        <button
                          onClick={deleteToFriend}
                          className="btn secondary"
                        >
                          Supprimer des amis
                        </button>
                      ) : (
                        <button onClick={addToFriend} className="btn secondary">
                          Ajouter aux amis
                        </button>
                      )}
                    </div>
